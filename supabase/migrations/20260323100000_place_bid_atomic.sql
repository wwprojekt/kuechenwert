-- Migration: Create place_bid_atomic RPC function
-- Purpose: Prevent race conditions in bid placement by using
--          pg_advisory_xact_lock to serialize concurrent bids
--          for the same auction.
--
-- The function:
-- 1. Acquires a transaction-scoped advisory lock keyed on the auction UUID
-- 2. Re-reads the auction state (current_bid, status, end_time) inside the lock
-- 3. Validates the bid amount against the actual current_bid (not stale client data)
-- 4. Validates auction status, end time, motorhome status, and seller != bidder
-- 5. Inserts the bid and updates the auction current_bid atomically
-- 6. Handles soft-close extension if within the 5-minute window
-- 7. Releases the lock automatically when the transaction commits
...
GRANT EXECUTE ON FUNCTION place_bid_atomic(UUID, UUID, NUMERIC, BOOLEAN, NUMERIC, NUMERIC) TO service_role;

-- Revoke from anon and authenticated to prevent direct client calls
REVOKE EXECUTE ON FUNCTION place_bid_atomic(UUID, UUID, NUMERIC, BOOLEAN, NUMERIC, NUMERIC) FROM anon;
REVOKE EXECUTE ON FUNCTION place_bid_atomic(UUID, UUID, NUMERIC, BOOLEAN, NUMERIC, NUMERIC) FROM authenticated;
