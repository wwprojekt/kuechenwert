...
export const motorhomeTechnicalSchema = z.object({
  ...
  power_ps: z.coerce.number().min(0).max(1500).optional().or(z.literal("")),
  ...
  next_tuev_date: z.string().optional(),
...
export const motorhomeDimensionsSchema = z.object({
  ...
  total_weight_kg: z.coerce.number().min(500).max(20000).optional().or(z.literal("")),
  payload_kg: z.coerce.number().min(0).max(5000).optional().or(z.literal("")),
  number_of_axles: z.coerce.number().min(1).max(4).default(2),
  seats_with_seatbelts: z.coerce.number().min(1).max(9).optional().or(z.literal("")),
...
export const motorhomeInteriorSchema = z.object({
  ...
  air_conditioning: z.enum(["Keine", "Fahrerhaus", "Wohnraum", "Beides"]).default("Keine"),
  has_bathroom: z.boolean().default(false),
  has_toilet: z.boolean().default(false),
  has_shower: z.boolean().default(false),
  fresh_water_capacity_liters: z.coerce.number().min(0).max(1000).optional().or(z.literal("")),
...
export const motorhomeEquipmentSchema = z.object({
  ...
  awning_length_cm: z.coerce.number().min(0).max(1000).optional().or(z.literal("")),
  has_bike_rack: z.boolean().default(false),
  has_garage: z.boolean().default(false),
  has_tv_sat: z.boolean().default(false),
  has_reversing_camera: z.boolean().default(false),
