  ...
  if (sendEmail) {
    ...
    if (seller?.email) {
      ...
      const content = `
        ...
        ${paragraph(
          'Falls Sie Ihr Fahrzeug erneut anbieten möchten, können Sie es im Verkäufer-Dashboard reaktivieren oder bei Fragen unter <a href="mailto:info@caravanwert.de" style="color:#1f8aa2;">info@caravanwert.de</a> auf uns zukommen.',
        )}
      `;
    ...
    for (const profile of bidderProfiles) {
      ...
      const content = `
        ...
        ${paragraph(
          'Schauen Sie sich gerne in unseren <a href="https://caravanwert.de/kaufen" style="color:#1f8aa2;">aktuellen Auktionen</a> nach einem ähnlichen Fahrzeug um.',
        )}
        ${paragraph(
          'Bei Rückfragen erreichen Sie uns unter <a href="mailto:info@caravanwert.de" style="color:#1f8aa2;">info@caravanwert.de</a>.',
        )}
      `;
      const html = buildEmailLayout(settingsData, subject, content);
