...
interface AutoConvertRequest {
  ...
  userId?: string;
  hasPassword?: boolean;
  // Used only for ownership verification of guest submissions. The caller
  // must send the same anonymous_id that was written to wizard_sessions.
  anonymousId?: string;
}
    ...
    if (authHeader?.toLowerCase().startsWith("bearer ")) {
      ...
      if (token && token !== SUPABASE_SERVICE_ROLE_KEY) {
        try {
          ...
          // Fall through -> guest flow, RPC below decides access.
        }
      }
    }
