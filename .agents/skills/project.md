...
### Offene Aufgaben
- [ ] **KRITISCH – restliche Functions mit `checkServiceRoleOrAdmin` neu deployen**: 37 Functions bündeln noch die alte `_shared/auth.ts` und akzeptieren gefälschte service_role-Tokens (u. a. `admin-delete-user`, `send-registration-invite`, `send-push-notification`, `process-dunning`, `send-welcome-email`). Die CLI war am 26.09.2026 nicht angemeldet (401), per MCP sind die Bundles zu groß. Mit Token: `supabase functions deploy --project-ref gzqayoalwtmypndrmqes --use-api` und danach Probe mit gefälschtem Token (muss 401 liefern).
...
- [ ] Caravan-Reste in Legacy-Bereichen entscheiden (behalten und umbauen oder ausblenden): Ausstellungsküchen `/kaufen` + `/auktion/:id` (Kilometerstand, Kraftstoff, „Ankaufspreis“), `ListingDetail`, `NotificationPreferences`, Dashboard-Deep-Routes (`auctions`, `inventory`, `claims`, `kaufchancen`, `search-alerts`, `appointments`, `contracts`), Admin-Seiten, Legacy-Mails/PDFs (Kaufvertrag, Übergabeprotokoll, Auktions-Mails), Fehlermeldungen in DB-Funktionen („Wohnmobil nicht gefunden“).
